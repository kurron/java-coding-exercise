...
Debugging streams can be awkward due to the fluent API which focuses on combining steps to construct a valid pipeline.  In order to get your stream to compile and process, you need to apply a terminal operator. Oftentimes, I would apply the terminal `forEach` function to dump the contents of the stream.  The use of method references allowed me to set break points as the algorithm progressed but I'm curious how IDEA would have handled a breakpoint in a one-line Lambda?
